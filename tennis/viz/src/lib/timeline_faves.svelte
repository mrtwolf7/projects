<script>
  ...
  $: {
    ...
    groups.forEach(([tourney, decadeBuckets]) => {
      decadeBuckets.forEach(([dec, values]) => {
        decadeLookup.set(`${tourney}-${dec}`, d3.mean(values, v => v.fave_ratio));
      });
    });
  }
  ...
  $: line = d3.line().x(d => x(d.year)).y(d => y(d.fave_ratio));
  ...
  function handleMouseMove(event) {
    ...
    if (Math.hypot(x(closest.year) - mx, y(closest.fave_ratio) - my) < 50) {
...
<div class="chart-container">
  <svg {width} {height} on:mouseleave={() => hoveredPoint = null}>
    {#if x && y && filteredData.length > 0}
      ...
      {#if hoveredPoint}
        <circle cx={x(hoveredPoint.year)} cy={y(hoveredPoint.fave_ratio)} r="6" fill={color(hoveredPoint.tournament)} stroke="white" stroke-width="2" pointer-events="none" />
      {/if}
