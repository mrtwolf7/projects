<script>
  ...
  $: {
    filteredData = rawData
      ...
      .map(d => ({
        ...
        avg_winner_games: +d.avg_winner_games,
    ...
    groups.forEach(([tourney, decadeBuckets]) => {
      decadeBuckets.forEach(([dec, values]) => {
        decadeLookup.set(`${tourney}-${dec}`, d3.mean(values, v => v.avg_winner_games));
      });
    });
  }
  ...
  $: if (filteredData.length > 0) {
    ...
    y = d3.scaleLinear()
      .domain(d3.extent(filteredData, d => d.avg_winner_games))
      .nice()
      .range([height - margin.bottom, margin.top]);

    color = d3.scaleOrdinal(d3.schemeCategory10).domain(tournamentList);
    delaunay = d3.Delaunay.from(filteredData, d => x(d.year), d => y(d.avg_winner_games));
  }

  $: line = d3.line().x(d => x(d.year)).y(d => y(d.avg_winner_games));
  ...
  function handleMouseMove(event) {
    ...
    if (Math.hypot(x(closest.year) - mx, y(closest.avg_winner_games) - my) < 50) {
...
<div class="chart-container">
  <svg {width} {height} on:mouseleave={() => hoveredPoint = null}>
    {#if x && y && filteredData.length > 0}
      ...
      {#if hoveredPoint}
        <circle cx={x(hoveredPoint.year)} cy={y(hoveredPoint.avg_winner_games)} r="6" fill={color(hoveredPoint.tournament)} stroke="white" stroke-width="2" pointer-events="none" />
      {/if}
